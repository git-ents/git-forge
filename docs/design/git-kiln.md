...
The index uses a flat ref-per-entry structure to avoid write contention:

```text
refs/kiln/plans/<worktree_hash>       → plan tree
refs/kiln/outputs/<action_hash>       → signed commit (tree = build output)
```

One ref per cached output.
Two CI jobs finishing simultaneously never conflict — they write to different refs.
...
```sh
git fetch origin refs/kiln/outputs/abc123
```
...
For Rust, `rust-analyzer` runs `cargo check` internally, which invokes `rustc --emit=metadata` on every crate.
Without restored state, this recompiles every dependency from scratch — minutes on a large workspace.
With restored state, `cargo check` skips all cached deps and only checks the crate the developer is editing.
...
The `.d` files (dependency tracking) and top-level binaries/rlibs are not needed for state restoration.
Cargo regenerates `.d` files from fingerprints, and top-level binaries are just the final link step — seconds to reproduce from cached `.rlib` files.
...
The planner reads these hashes and declares them on the fetch action's `expects` field:
...
[expects]
crates = "sha256:ab34ef..."
```

After execution, kiln verifies the output tree against the expected hashes.
If they match, the output is accepted.
If they don't, the output is rejected and the build fails.
...
Build actions downstream of the vendor node run at the policy-required isolation level.
The vendor output is their input.
...
If a cached vendor output exists but its hash doesn't match the expected value, kiln treats it as a miss and re-fetches.
Cache poisoning is caught.

Fetch actions — including the vendor node — should be single coarse-grained actions per lock file.
`cargo vendor --locked` fetches everything in one pass.
Splitting into per-crate fetch actions adds hundreds of network-dependent nodes with marginal savings — fetches are cheap, compilation is expensive.
Git deduplicates unchanged crate source blobs across vendor runs at the blob level.
One vendor node per lock file is the right granularity.
...
9. Traverse the full plan DAG.
   For each active node, compute the action hash (which includes the resolved profile-specific fields), check for a cached output.
   On a hit, skip.
   On a miss, build.
   For nodes with `expects`, verify output hashes after execution.
10. Store artifact outputs as refs in the index.
...
Cargo's fingerprinting is mtime-based.
It records the modification time and absolute path of every source file and artifact.
When fingerprints are restored from a remote cache onto a different machine, mtimes and paths will not match unless the executor enforces determinism.
This is the single hardest integration problem for `cargo-kiln`.

Kiln's executor must normalize mtimes and paths before invoking cargo.
This is a contract, not an optimization — without it, cargo recompiles everything on every cache restore, defeating the purpose of the system.

**Path determinism.**
The executor must ensure that cargo always runs from a canonical working directory.
At isolation level two and above, this is trivial — the sandbox mounts the worktree at a fixed path (e.g., `/kiln/workspace/`).
Fingerprints are recorded against that path on every machine.
At levels zero and one, the executor creates a symlink from a fixed canonical path to the actual checkout and invokes cargo from there:
...
This only works if kiln is the sole entry point for builds.
If the developer runs `cargo build` directly from their checkout path, cargo records fingerprints against the real path, clobbering the kiln-compatible ones.
Kiln should detect non-canonical fingerprint paths and warn.

**Mtime determinism.**
Before every cargo invocation, kiln sets all source file mtimes and all restored artifact mtimes to a deterministic value — the git commit timestamp of HEAD.
The value itself does not matter as long as it is reproducible.
Both CI and the local machine perform the same normalization:

```sh
git ls-files | xargs touch -t <commit_timestamp>
```

For restored artifacts, kiln sets mtimes during the restore step, before cargo ever sees them.
Fingerprints were recorded on CI against the same normalized mtimes.
The check passes.

**Full cache hit path.**
When every action in the DAG is a cache hit, kiln never invokes cargo at all.
Kiln checks action hashes, all hit, restores outputs.
There are no fingerprints to consult.
The mtime normalization contract only matters for partial rebuilds — when some crates are cache hits and one is modified, and kiln needs cargo to rebuild just the modified crate without recompiling its cached dependencies.

**Limitation: fingerprint internal format.**
Cargo serializes its own structs into `.fingerprint/` with embedded mtime data and path hashes.
The format is not stable across cargo versions.
Kiln does not patch fingerprint internals — it ensures the conditions under which fingerprints were created are reproduced exactly.
If cargo changes its fingerprint format, the existing cache entries become misses, which is correct behavior — a toolchain change invalidates the cache.

**Future direction: direct rustc invocation.**
If cargo's fingerprinting proves too fragile, `cargo-kiln` can bypass cargo for execution entirely.
The planner already calls `cargo metadata` for the dependency graph and can capture exact `rustc` invocations from `cargo build -v`.
Each action's `run` field becomes a direct `rustc` call with `--extern` flags pointing at dependency `.rlib` inputs.
This is what Buck2 and Bazel's `rules_rust` do.
It eliminates the fingerprint problem but requires maintaining compatibility with rustc's unstable CLI surface.
This is deferred unless mtime normalization proves insufficient.
...
Top-level binaries and rlibs (`target/debug/my-binary`, `target/debug/libmy_crate.rlib`) are just the final link step from `.rlib` files in `deps/` — seconds to reproduce.
`.d` files (dependency tracking) are regenerated from fingerprints.
The `incremental/` directory is the incremental compilation cache — large, fragile, and inherently non-hermetic.
The `examples/` directory is built on demand.
`.cargo-lock`, `.rustc_info.json`, and `CACHEDIR.TAG` at the target root are metadata files cargo recreates.

`cargo-kiln` stores `deps/` + `.fingerprint/` + `build/` per action.
It does not store `incremental/` — incremental compilation is a local convenience that conflicts with hermetic caching.
The `state` output restores these three directories after a build so that subsequent cargo invocations (including rust-analyzer's `cargo check`) see a warm cache.
...
`cargo-kiln` calls `cargo metadata`.
...
The adapter emits fetch actions alongside build actions.
For Rust, `cargo-kiln` reads `Cargo.lock`, emits a vendor action at isolation level zero, and declares expected output hashes derived from the lock file's per-crate content hashes:
...
Build actions depend on the vendor action's output:
...
The `--frozen` flag tells Cargo not to touch the network or update the lock file.
All crates are already local in the vendored tree.
At isolation level three, the network is blocked anyway — but the flag makes the intent explicit even at lower levels.

The adapter doesn't fetch anything.
It declares nodes.
Kiln executes the vendor action (or retrieves a cache hit), verifies the output against expected hashes, and stages the vendored crates as inputs to downstream build actions.
The same pattern applies to every language with a lock file: `uv-kiln` emits a vendor action reading `uv.lock`, `go-kiln` emits one reading `go.sum`.
...
At the compilation unit level, one node per compiler invocation achieves maximum caching but effectively replaces the language's own build orchestration.

For most languages, the module level is the practical sweet spot.
The language tool already thinks in these units.
The planner uses standard introspection APIs.
And the language's native build system still runs inside the sandbox, handling the fine-grained details.
...
Each build step produces a signed commit rather than just a tree.
The commit's tree is the build output.
The commit's parents are the input commits — the dependencies that were built.
The commit message contains metadata: the action, the isolation level, the toolchain hash, the source hash.

The parent chain is the build graph.
A CLI binary's output commit has its parser and core library as parents.
`git log` on the build graph shows the entire DAG.
`git verify-commit` on any node proves CI built it.
`git diff` between two builds shows exactly what changed.
Bisecting a build regression is `git bisect` on the build graph.
...
The vendored source tree (from the vendor action) provides an even stronger guarantee — the exact source used to produce the binary is in the same Git object store, fetchable by hash.

### Vendored Source in the Cache

The vendor action's output tree contains copies of every dependency's source.
Redistributing this tree is redistributing those dependencies' source code.
For permissive licenses, this is fine with attribution.
For copyleft licenses, this is actually desirable — GPL requires source availability, and the vendor tree provides it.
...
The planner also emits vendor fetch actions with expected output hashes derived from the lock file.
